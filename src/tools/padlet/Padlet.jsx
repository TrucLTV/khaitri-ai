...
/* ════════════════════════════════════════
   GV — Tạo bảng
════════════════════════════════════════ */
function CreateBoard({ onCreate, user }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        <div className="flex items-start gap-4">
          ...
          {isTeacher && (
            <div className="flex-shrink-0 text-center">
              <button onClick={() => setShowQR(s => !s)} className="text-xs text-gray-400 hover:text-primary-600 mb-1 block">
        ...
        {isTeacher && (
          <button onClick={handleClose} className="mt-3 text-xs text-gray-400 hover:text-red-500">
            Đóng bảng
          </button>
        )}
      </div>
            ...
            <div key={card.id}
              ...
              {isTeacher && (
                <button onClick={() => handleDelete(card.id)}
                  className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 text-sm transition-opacity">×</button>
              )}
            </div>
          ))}
        </div>
...
function StudentPost({ code: initCode }) {
  ...
  const join = async (c) => {
    ...
    try {
      const snap = await getDoc(doc(db, 'padlet', key))
  ...
  if (posted) return (
    <div className="max-w-sm mx-auto space-y-4">
      ...
      <button onClick={() => { setContent(''); setPosted(false); setColorIdx(Math.floor(Math.random() * CARD_COLORS.length)) }}
        ...
        <div className="flex gap-1.5 mb-2">
          {CARD_COLORS.map((cls, i) => (
            <button key={i} onClick={() => setColorIdx(i)}
              className={`w-6 h-6 rounded-full border-2 ${cls.split(' ')[0].replace('100', '300')} ${colorIdx === i ? 'border-gray-700 scale-125' : 'border-transparent'} transition-transform`} />
          ))}
        </div>
        <textarea value={content} onChange={e => setContent(e.target.value)}
          placeholder="Câu trả lời của bạn..." rows={4}
          className={`w-full border rounded-xl px-4 py-3 text-sm focus:outline-none resize-none ${CARD_COLORS[colorIdx]}`} />
      </div>
      <button onClick={submit} disabled={!name.trim() || !content.trim()}
...
function TeacherPanel() {
  ...
  if (!code) return (
    <CreateBoard user={user} onCreate={async (c) => {
      setCode(c)
  ...
  if (code && board) return (
    <Board code={code} boardData={board} cards={cards} isTeacher={true}
      onClose={() => { setCode(null); setBoard(null) }} />
  )
...
export default function Padlet({ studentCode }) {
  ...
  if (studentCode || mode === 'student') return <StudentPost code={studentCode || ''} />
